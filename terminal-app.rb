...
def recommend_search  #user search function to return recommendations
	recommend_array = [] #empty array to be filled with recommendations		
	until recommend_array.length == 3 #while less than three uniue items in array then loop
		puts "enter a book"
		puts ">"
		book = gets.chomp
		if !recommend_array.include?(book)
		recommend_array << book
	end
	return recommend_array
end
recommendations = recommend_search
p recommendations

	# author_array.each do |author| 
	# 	if author.name.include?(search_term)
	# 		#find genre by searched author
	# 		#find and select title within genre
	# 		#return title and push to recommend_array 
  #     recommend_array<<"#{author.name} (author)"#push other author
    
	# 	book_array.each do |book|
	# 	if book.title.include?(search_term)
	# 		#find genre by searched title
	# 		#find and select another title within genre
	# 		#return title and push to recommend_array
	#     recommend_array<<"#{book.title} (book) #{book.genres}"
